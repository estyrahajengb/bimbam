...
@user_passes_test(is_user)
def level(request):
    return render (request, 'pembelajaran/level.html')
    ...
    if tingkat in choices:
        if tingkat == 'Tingkat 2':
            ...
            if not pengajuan_tingkat3 or pengajuan_tingkat3.status != 'Disetujui':
                ...
                return redirect('materimurid')
            video = Video.objects.filter(Q(materi='Menghitung Tingkat 2') | Q(materi='Menghitung Tingkat 3'))
        else:
...
def membaca(request, tingkat):
    ...
    if tingkat in choices:
        if tingkat == 'Tingkat 2':
            ...
            if not pengajuan_tingkat2 or pengajuan_tingkat3.status != 'Disetujui':
            ...
            if not pengajuan_tingkat3 or pengajuan_tingkat3.status != 'Disetujui':
                messages.error(request, 'Anda belum diizinkan untuk mengakses tingkat 3.')
                return redirect('materimurid')
            video = Video.objects.filter(Q(materi='Membaca Tingkat 2') | Q(materi='Membaca Tingkat 3'))
        else:
            video = Video.objects.all()
        if video.exists():
    ...
    if tingkat in choices:
        if tingkat == 'Tingkat 2':
            ...
            if not pengajuan_tingkat3 or pengajuan_tingkat3.status != 'Disetujui':
                messages.error(request, 'Anda belum diizinkan untuk mengakses tingkat 3.')
                return redirect('materimurid')
            video = Video.objects.filter(Q(materi='Menulis Tingkat 2') | Q(materi='Menulis Tingkat 3'))
        else:
            video = Video.objects.all()
        if video.exists():
...
# @login_required(login_url=settings.LOGIN_URL)
# @user_passes_test(is_user)
# def proses_jawaban(request):
#     if request.method == 'POST':
#         user = request.user  # Mendapatkan informasi pengguna terautentikasi

#         jawaban_pengguna = {}

#         for key, value in request.POST.items():
#             if key.startswith('jawaban_'):
#                 soal_id = key.split('_')[1]
#                 jawaban_pengguna[soal_id] = value

#         # Simpan jawaban pengguna ke database dengan menyertakan informasi pengguna
#         for soal_id, jawaban in jawaban_pengguna.items():
#             soal = Soal.objects.get(id=soal_id)

#             if jawaban.lower() == soal.jawaban_benar.lower():  # Membandingkan jawaban pengguna dengan jawaban benar (tanpa memperhatikan huruf besar/kecil)
#                 benar = "Benar"
#             else:
#                 benar = "Salah"

#             jawaban_murid = JawabanMurid(soal=soal, jawaban_pengguna=jawaban, user=user, benar=benar)
#             jawaban_murid.save()
#         messages.success(request, 'Soal anda sudah terkirim')
...
# @login_required(login_url=settings.LOGIN_URL)
# @user_passes_test(is_user)
# def soal(request, materi):
#     soal = Soal.objects.filter(materi=materi)
#     return render(request, 'soall.html', {'soal': soal})

# @login_required(login_url=settings.LOGIN_URL)
# @user_passes_test(is_user)
# def hasiljawaban(request, id):
#     jawaban = JawabanMurid.objects.filter(id=id)
#     return render(request, 'hasiljawaban.html', {'jawaban':jawaban})